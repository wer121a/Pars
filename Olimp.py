    ...
    while True:
        ...
        if pp.find_elements(By.CLASS_NAME, 'content--V86vO'): # Условие, если есть событие, продолжаем парсить, если нет, цикл будет запускаться заново, пока не появится событие
            ...
            for sob_futbal in conteiner_futball: # Обращаемся к каждому событию
                ...
                if sob_futbal.find_element(By.CLASS_NAME, 'name--R2XME').text.replace('\n',''):
                    ...
                    name_futball = sob_futbal.find_element(By.CLASS_NAME, 'name--R2XME').text
            ...
            travel.put(base_olimp.copy()) # Передаём сформированный df с коэф. в Queue (добавляем в очередь, из которой он будет взят в файле match)
